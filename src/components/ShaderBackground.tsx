...
{
  float r = 0.015; // Edge rounding radius for less blocky buildings
  vec3 q = abs(p) - b + r;
...
DistBuilding distCars(vec3 po) {
    ...
    vec3 carBody = vec3(carSize*2., carSize, carSize);
    float d = sdBox(q1, carBody);
    // Add a cabin on top
    vec3 cabinQ = q1 - vec3(0.0, 0.0, carSize * 0.8);
    float dCabin = sdBox(cabinQ, vec3(carSize * 1.2, carSize * 0.8, carSize * 0.7));
    d = min(d, dCabin);
    ...
    // Rounded car body shape (vertical direction)
    vec3 carBody2 = vec3(carSize, carSize * 2., carSize);
    float d1 = sdBox(q1, carBody2);
    vec3 cabinQ2 = q1 - vec3(0.0, 0.0, carSize * 0.8);
    float dCabin2 = sdBox(cabinQ2, vec3(carSize * 0.8, carSize * 1.2, carSize * 0.7));
    d1 = min(d1, dCabin2);
...
DistBuilding distTrafficLights(vec3 po) {
    DistBuilding res;
    res.d = 100000.0;
    res.objId = OBJ_TRAFFIC_LIGHT;
    res.q1 = vec3(0.0);
    res.size = vec3(0.0);
    res.height = 0.0;

    // Find nearest 4-way street intersection center (halfway between building grid cells)
    vec2 interID = floor(po.xy / CELL_SIZE);

    if (abs(interID.x) > bounds.x || abs(interID.y) > bounds.y) {
        return res;
    }

    vec2 interCenter = (interID + vec2(0.5)) * CELL_SIZE;
    vec2 ip = po.xy - interCenter; // Pos relative to 4-way intersection center

    // Determine corner quadrant (4 sidewalk corners around intersection)
    vec2 cornerSign = sign(ip);
    if (cornerSign.x == 0.0) cornerSign.x = 1.0;
    if (cornerSign.y == 0.0) cornerSign.y = 1.0;

    // Sidewalk corner placement (anchored on sidewalk corner back from street intersection)
    vec2 cornerPos = cornerSign * 0.26;
    vec3 pLocal = vec3(ip - cornerPos, po.z);

    // Vertical pole (height 0.25)
    float poleH = 0.25;
    float dPole = sdBox(pLocal - vec3(0.0, 0.0, poleH * 0.5), vec3(0.008, 0.008, poleH * 0.5));

    // Orient signal overhang arm over East-West vs North-South road lanes
    bool isYAxis = (cornerSign.x * cornerSign.y < 0.0);

    vec3 armOffset, armSize, boxOffset, boxSize;
    if (isYAxis) {
        armOffset = vec3(0.0, -cornerSign.y * 0.045, poleH - 0.02);
        armSize = vec3(0.005, 0.045, 0.005);
        boxOffset = vec3(0.0, -cornerSign.y * 0.08, poleH - 0.04);
        boxSize = vec3(0.015, 0.015, 0.035);
    } else {
        armOffset = vec3(-cornerSign.x * 0.045, 0.0, poleH - 0.02);
        armSize = vec3(0.045, 0.005, 0.005);
        boxOffset = vec3(-cornerSign.x * 0.08, 0.0, poleH - 0.04);
        boxSize = vec3(0.015, 0.015, 0.035);
    }

    float dArm = sdBox(pLocal - armOffset, armSize);
    float dBoxSig = sdBox(pLocal - boxOffset, boxSize);

    float dTotal = min(dPole, min(dArm, dBoxSig));
...
DistResult getDist(vec3 p) {
    ...
    DistBuilding tlights = distTrafficLights(p);
    if (tlights.d < d) {
        d = tlights.d;
        obj = OBJ_TRAFFIC_LIGHT;
    }
...
// Cheap ambient occlusion - samples distance field at offset points along the normal
float calcAO(vec3 p, vec3 n) {
    float ao = 0.0;
    float scale = 1.0;
    for (int i = 0; i < 3; i++) {
        float h = 0.02 + 0.06 * float(i);
        float d = getDist(p + n * h).d;
        ao += (h - d) * scale;
        scale *= 0.5;
    }
...
vec4 floorTexture(vec3 p, vec3 q1) {
    ...
    if (abs(blockID.x) > bounds.x || abs(blockID.y) > bounds.y) {
        // Procedural grass texture instead of flat color
        vec3 grassCol = COLOR_BUILDING_BASE;
        float grassNoise = noise(p.xy * 8.0, 4);
        grassCol *= 0.7 + grassNoise * 0.6; // Color variation
        // Fine grass blade pattern
        float blades = noise(p.xy * 60.0, 2) * 0.3;
        grassCol += blades * vec3(0.05, 0.12, 0.02);
        // Subtle darker patches for clumps
        float clumps = smoothNoise(p.xy * 3.0);
        grassCol *= 0.85 + clumps * 0.3;
        return vec4(grassCol, fogMultiplier);
    }
    ...
    vec2 baseUV = abs(uv);
    float isBase = step(max(baseUV.x, baseUV.y), BULDING_BASE_SIZE*.9);

    // Walkway paths from buildings to roads
    float pathWidth = 0.03;
    float bn = n21(blockID);
    float bn2 = fract(bn * 347.7);
    float isPath = 0.0;
    // Path going in X direction (horizontal) - most buildings get one
    if (bn > 0.2) {
        float pathOffsetY = (bn2 - 0.5) * BULDING_BASE_SIZE * 0.6;
        float inPathX = step(abs(uv.y - pathOffsetY), pathWidth);
        float beyondBase = step(BULDING_BASE_SIZE * 0.9, abs(uv.x));
        isPath += inPathX * beyondBase;
    }
    // Path going in Y direction (vertical) - some buildings get a second
    if (bn2 > 0.4) {
        float pathOffsetX = (fract(bn * 891.3) - 0.5) * BULDING_BASE_SIZE * 0.6;
        float inPathY = step(abs(uv.x - pathOffsetX), pathWidth);
        float beyondBaseY = step(BULDING_BASE_SIZE * 0.9, abs(uv.y));
        isPath += inPathY * beyondBaseY;
    }
    isPath = min(isPath, 1.0);

    if (isBase > 0.5) {
        // Textured grass for building plots
        vec3 plotGrass = COLOR_BUILDING_BASE;
        float gn = noise(p.xy * 12.0, 3);
        plotGrass *= 0.75 + gn * 0.5;
        float blades = noise(p.xy * 50.0, 2) * 0.25;
        plotGrass += blades * vec3(0.04, 0.1, 0.02);
        col += plotGrass;
    }
    ...
    // Draw paths over sidewalk areas (concrete colored)
    if (isPath > 0.5 && road < 0.5 && isBase < 0.5) {
        col = vec3(0.55, 0.52, 0.48); // Concrete path color
    }
...
vec3 trafficLightTexture(TraceResult tr, vec3 normal) {
    vec3 col = vec3(0.15);
    vec3 pLocal = tr.q1;
    float poleH = 0.25;

    if (pLocal.z > poleH - 0.055 && pLocal.z < poleH - 0.005) {
        float dirOffset = (tr.id.x + tr.id.y);
        float signalPhase = mod(iTime * 1.5 + dirOffset * 2.5, 6.0);
        float lightZ = pLocal.z - (poleH - 0.03);

        // Red light (top)
        if (lightZ > 0.005 && signalPhase < 2.6) {
            col = vec3(3.5, 0.15, 0.15); // Emissive bright red glow
        }
        // Yellow light (middle)
        else if (abs(lightZ) <= 0.005 && signalPhase >= 2.6 && signalPhase < 3.4) {
            col = vec3(3.2, 2.5, 0.15); // Emissive bright yellow glow
        }
        // Green light (bottom)
        else if (lightZ < -0.005 && signalPhase >= 3.4) {
            col = vec3(0.15, 3.5, 0.5); // Emissive bright green glow
        }
    }
    return col;
}
...
void scriptCamera() {
    ...
    // Smooth, continuous cinematic camera trajectory
    camera.x = sin(t * 1.1) * 6.0 + cos(t * 0.3) * 2.0;
    camera.y = cos(t * 0.8) * 6.0 + sin(t * 0.4) * 2.0;

    // Elevate camera higher above the city for a broader aerial view
    camera.z = 2.5 + sin(t * 1.2) * 0.3;

    // Angled camera tilt looking down over the city skyline
    camera.horizontalAngle = sin(t * 0.8) * 0.35;
    camera.verticalAngle = 0.7 + sin(t * 0.5) * 0.05;
    camera.rotation = sin(t * 0.4) * 0.02;
}
...
void mainImage(out vec4 fragColor, in vec2 fragCoords) {
    ...
    if (tr.dt < MIN_DISTANCE) {
        ...
        if (objId == OBJ_FLOOR) {
            ...
            fogMultiplier = 2.;
        } else if (objId == OBJ_TRAFFIC_LIGHT) {
            normal = getNormal(tr.p, tr.dt);
            col += trafficLightTexture(tr, normal);
            fogMultiplier = 1.5;
        } else {
            normal = getNormal(tr.p, tr.dt);
        ...
        // Subtle directional lighting for 3D depth and form
        if (objId != OBJ_DOME && objId != OBJ_CAMERA) {
            vec3 lightDir = normalize(vec3(0.4, 0.7, -0.6));
            float diff = max(dot(normal, lightDir), 0.0);
            float ambient = 0.4;
            float ao = calcAO(tr.p, normal);
            col *= (ambient + diff * 0.6) * ao;

            // Subtle rim light on building edges for definition
            float rim = pow(1.0 - max(dot(normal, -rd), 0.0), 3.0) * 0.15;
            col += rim;
        }
